jQuery(document).ready(function($){
	if( $('.cd-form').length > 0 ) {
		...
		formPopup.on('click', 'a', function(event){
			...
			triggerAnimation( $(this).parents('.cd-pricing-footer').parent('li'), coverLayer, true, event.target.id);
		});
	...
	function triggerAnimation(table, layer, bool, id) {
		if( !animating ) {
			layer.toggleClass('is-visible', bool);
			animateForm(table, bool, id);
			table.toggleClass('selected-table', bool);
            /*var popupSetColor = $('.cd-pricing-features em');
            if(popupSetColor.hasClass('popup-font-gray')){
                popupSetColor.removeClass('popup-font-gray');
                $('.cd-pricing-features em span').removeClass('popup-font-white');
            }else{
                popupSetColor.addClass('popup-font-gray');
                $('.cd-pricing-features em span').addClass('popup-font-white');
            }*/
            if($('.wrap').hasClass('menu-animation')){
                pageReadyDel();
            }
            $('.cd-pricing-features>ul').toggleClass('open', bool);

		}
	}

	function animateForm(table, animationType, popupId) {
		animating = true;
		...
		var tableWidth = table.width(),
			...
			form = $('#'+popupId+'-code'),
			formPlan = $('.cd-form').find('.cd-plan-info'),
